...
■ すぐ鳴る・すぐ作れる
・ダイアトニックコードを候補から選ぶだけ（無料）
・全12メジャーキーに対応。キー変更も移調も無料
・三和音／7thの切り替え、sus4・sus2・add9などにも対応
...
■ 縦動画で共有
・作ったコード進行を9:16の縦動画に書き出し（無料は720p）
・再生中のコードと鍵盤ハイライトが同期
...
・打ち込みらしさを抑えた、手で弾いたようなタイミングとストローク
・1080pの高画質書き出し・透かしなし
・プロジェクトの保存数とお気に入りが無制限（無料は保存5件・お気に入り20件）
・いつでも解約可能。購入の復元にも対応
...
・高度なコード（テンション／借用和音／セカンダリードミナント／オンコード）と Pro プリセットが Palette Pro の対象です。キー変更・移調は無料で全12キー使えます。
