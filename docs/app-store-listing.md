...
・ブロック／8ビート／16ビート／アルペジオなどの伴奏パターン
・ポップ・ロック・ソウルなどのドラムグルーヴを重ねてループ再生
...
・6th／借用和音／セカンダリードミナント／オンコードなどの高度なコード
...
- `[要ユーザー]` 利用規約(EULA)URL・プライバシーポリシーURL（サブスクの法定表記に必要）。
...
| 項目 | 状態 |
|---|---|
| プライバシーポリシー URL | `[要ユーザー]` 公開必須（サブスクありのため） |
| サポート URL | `[要ユーザー]` |
| マーケティング URL（任意） | `[要ユーザー]` |
| App アイコン（1024×1024, 透過なし） | 既存 `assets/icon/app-icon.png` を元に生成 |
