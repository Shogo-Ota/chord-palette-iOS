...
- [ ] Development Build が生成でき、実機で起動する
- [ ] 検証専用画面が開く
- [ ] `ChordAudio.getVersion()` / `isAvailable()` が値を返す（モジュール到達確認）
- [ ] コード単体試聴（Cmaj7 等）が鳴る
- [ ] Cmaj7→G7→Am7→Fmaj7（各1小節/120BPM）が再生される
- [ ] Pop 8beat 最小ドラムが鳴る
- [ ] コードとドラムが**ずれずに同期**する
- [ ] **30秒以上**ループしても継ぎ目で破綻しない
- [ ] Chord / Drum / Master の音量変更が即座に反映
- [ ] 再生位置イベントで現在コードのハイライト（検証画面）が進む
- [ ] **停止後の再再生**（stop → play）が正しく動く
- [ ] **一時停止後の再開**（pause → resume）が保持位置から動く
- [ ] **10回以上の連続再生・停止**でクラッシュ/音欠け/状態不整合がない
- [ ] **ループ10周以上**で継ぎ目破綻・累積ズレがない
- [ ] **音量 0.0 と 1.0**（Master/Chord/Drum 各々）で正しく無音/最大になる
- [ ] **画面遷移後にエンジンが解放**される（`teardown`、リークしない）
- [ ] **Fast Refresh 後に二重エンジンが生成されない**（番兵/シングルトンで防止）
