...
| エンジン分離 | **Music Theory Engine**（本稿以下）と **Groove Engine**（`Groove.md` 以下）を分ける |
| 投資順 | 理論 → グルーヴ → ボイシング → AI メロディ（詳細は `Groove.md` §2） |
| 理論 vs 使いやすさ | 「理論上存在する」ことと「UI に出す」ことは別判断。実用性・ポップス頻度を優先 |

### 2.1 理論の正しさと使いやすさ

- 理論的に正しいコードでも、実用性が低いものは **表示しない／優先度を下げる** ことがある  
- J-POP・シティポップで頻繁に使うコードは **優先表示** してよい  
- 個別の採用/除外理由は `Extensions.md` / `ChordDefinitions.md` に一文残す  
- Cursor / エージェントは「理論的に全部入れる」方向へ暴走しないこと
