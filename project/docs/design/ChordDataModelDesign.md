...
```text
src/lib/music/
  definitions/catalog.ts   … ChordDefinition[]
  resolve.ts               … key 展開
  midi.ts                  … intervals → MIDI
  transpose.ts             … 既存移設
project/docs/music/*       … 人間可読の正（カタログと同期）
```

`src/data/music.ts` はファサードに縮小（UI 互換関数を再エクスポート）。
...
最小化方針: **新規 `src/lib/music/` を追加**し、旧 API をアダプタで維持してから内部を入れ替える。
